...
import { useGetUsersMutation,useGetCompaniesMutation,useGetRolesMutation,useAssignRolesToUsersMutation } from "../slices/userApiSlice"
import { useSelector } from "react-redux"
import {selectCurrentUserRoles /*, selectCurrentUserPermissions*/ }from '../../auth/slices/authSlice'
import { Col, Row,Button,message,Descriptions, Form , Input, Table,Cascader,Checkbox } from 'antd';
import {
  EditOutlined
...
const [assignRolesToUsers] = useAssignRolesToUsersMutation()
...
const onFinish = async (values) => {
  ...
  await assignRolesToUsers({userIds ,roles}).unwrap()
  ...
  reset()
    ...
    setTimeout(() => {
      ...
      message.error({
        content: 'must choose users and roles to proceed',
        key,
...
const showLoadingMessage  = (msg)=>{
  message.loading({
    ...
    const onRolesChange = (checkedValues) => {
  ...
  return (
   <>
   <Descriptions title="Assign roles to users"></Descriptions>
  ...
  <div
        ...
        <Descriptions title="Choose roles"></Descriptions>
    ...
    >
      ...
      <Checkbox.Group  onChange={onRolesChange} >
      <Row>
      {rolesOptions.map(x=>(
         <Col span={4}>
         <Checkbox value={x.value}>{x.value}</Checkbox>
       </Col>
      ))}
      </Row>
      </Checkbox.Group>
